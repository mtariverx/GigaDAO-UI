...
export type Governance = {
    councillors: Array<PublicKey>, approval_threshold: number, proposed_signers: Array<boolean>, proposal_is_active: boolean, proposal_type: ProposalType,
    proposed_councillors?: Array<PublicKey>, proposed_approval_threshold?: number,
    proposed_deactivation_stream?: PublicKey,
    proposed_withdrawal_amount?: number, proposed_withdrawal_receiver: PublicKey, proposed_withdrawal_stream?: PublicKey,
    num_streams: number
}
...
export type ConnectToStream = (nft: Nft, stream: Stream) => Promise<{nft: Nft, stream: Stream}>;
export type DisconnectFromStream = (nft: Nft, conn: Connection, stream: Stream) => Promise<{nft: Nft, conn:Connection, stream: Stream}>;
export type ClaimFromStream = (conn: Connection, stream: Stream) => Promise<{conn: Connection, stream: Stream}>;

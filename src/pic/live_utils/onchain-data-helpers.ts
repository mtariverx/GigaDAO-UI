...
import {CONNECTION_PDA_SEED, initProgram, STAKE_PDA_SEED} from "./rpc_helpers";
...
export async function checkIfStakeExists(nft: pic.Nft, wallet, network, ownerAddress, nftMintPubkey): Promise<{stakeAccountStatus: StakeAccountStatus, stakeAddress: PublicKey | undefined, numConnections?: number}> {
    ...
    try{
        ...
        let accountDoesNotExistError = e.toString().includes('Account does not exist');
        if (!accountDoesNotExistError){
            alert("got unknown type of error: " + e);
        }
        stakeAccountStatus = StakeAccountStatus.NOT_INITIALIZED;
    }
    return {stakeAccountStatus: stakeAccountStatus, stakeAddress: stake_pda, numConnections: numConnections};
}

export async function checkIfConnectionExists(wallet, network, stake_pda, streamAddress, decimals, dailyStreamRateRelative): Promise<pic.Connection | undefined> {
    let program = await initProgram(wallet, network);
    ...
    try{
        const connectionAccount = await program.account.connection.fetch(connection_pda);
...
export async function getStream(wallet, network, stream: pic.Stream){
    ...
    let tokenMint: spl_token.Mint = await spl_token.getMint(program.provider.connection, tokenMintAddress);
    let poolAccount = await spl_token.getAccount(program.provider.connection, tokenPoolAddress);
